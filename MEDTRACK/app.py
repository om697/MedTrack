...
if USE_DYNAMODB:
    # Force boto3 to use AWS CLI credentials
...
SNS_TOPIC_ARN = "arn:aws:sns:us-east-1:457456730571:Medtrack"
...
def doctordashboard():
    ...
    doctor_appts = [a for a in appointments if a.get('doctor','').strip().lower() == name.strip().lower()]
    now = datetime.now()
...
def patientdashboard():
    ...
    appointments = load_appointments()
    user_appointments = [appt for appt in appointments if appt.get('patient','') == name]    
    now = datetime.now()
